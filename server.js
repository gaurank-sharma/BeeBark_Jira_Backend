...
const connectCloudinary = require('./config/cloudinary'); // Cloudinary Config
...
// --- CRON JOB: OVERDUE CHECK (Daily 9 AM) ---
cron.schedule('0 9 * * *', async () => {
    console.log("⏰ Running Daily Deadline Check...");
    try {
        const overdueTasks = await Task.find({
        ...
        for (const task of overdueTasks) {
            if (task.assignee?.email) {
                await sendEmail(
                    task.assignee.email,
                    `Running Late: ${task.title}`,
                    `<h3 style="color:#d32f2f;">⚠️ Task Overdue</h3><p>Due: <b>${new Date(task.deadline).toDateString()}</b></p><p>${task.title}</p>`
                );
            }
        }
...
app.post('/api/tasks', authenticateToken, upload.array('files'), async (req, res) => {
  try {
    ...
    const attachments = req.files ? req.files.map(f => ({
        url: f.path,
        public_id: f.filename,
        format: f.mimetype, // Important for PDF detection
        name: f.originalname
    })) : [];
    ...
    if (populatedTask.assignee?.email) {
        await sendEmail(populatedTask.assignee.email, `[JIRA] Assigned: ${title}`, 
            `<h3>Hello ${populatedTask.assignee.username},</h3><p><b>${populatedTask.reporter.username}</b> assigned this task to you.</p>`);
    }
    res.json(populatedTask);
  } catch (err) {
...
app.get('/api/tasks', authenticateToken, async (req, res) => {
    try {
        const { filter } = req.query;
        let query = {};
        if (filter === 'my-tasks') query.assignee = req.user._id;
...
app.put('/api/tasks/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const uniqueRecipients = [...new Set(recipients)];
        for (const email of uniqueRecipients) {
            await sendEmail(email, `[JIRA] Update: ${task.title}`, `<p>Update on <b>${task.title}</b>.</p>`);
        }
        res.json(task);
